...
- 单个源文件最大 64 MiB。
- 相册/文件选择器返回的内容 URI 只打开一次，并流式复制到应用私有缓存；后续 EXIF、缩略图和模型图解码均读取该缓存，以兼容 Vivo 等采用临时安全访问流的文件选择器。
...
| 真机安装与启动 | vivo V2359A / Android 16 安装、冷启动通过 |
| 真机图片预处理 | Vivo 安全文件选择器读取通过；暗化圆环、原生预填充与真实 100% 状态通过 |
